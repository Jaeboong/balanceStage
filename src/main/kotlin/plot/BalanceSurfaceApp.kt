...
class BalanceSurfaceApp : Application() {
    ...
    private val planeEstimator: PlaneEstimator = PcaPlaneEstimator()
    private val distanceCalc: DistanceCalculator = DefaultDistanceCalculator()
    ...
    override fun start(primaryStage: Stage) {
        ...
        val ZgColor = rbfGaussianEval(w, x, y, Xg, Yg, sigma)

        val (vmin, vmax) = if (forceVminZero)
            0.0 to max(v.maxOrNull() ?: 1.0, 1e-9)
        else {
            val zmin = ZgColor.min2D(); val zmax = ZgColor.max2D()
            zmin to if (abs(zmax - zmin) < 1e-9) zmin + 1 else zmax
        }

        // === PCA 평균평면 (z = -(nx x + ny y + d)/nz) ===
        val plane = planeEstimator.estimate(x, y, z)
        val (nx, ny, nz) = plane.normal
        val ZgHeight = planeEstimator.planeZ(plane.normal, plane.d, Xg, Yg) // 평면 높이(=z)

        // 거리 로깅
        val signed = distanceCalc.signedDistances(points.map { doubleArrayOf(it[0], it[1], it[2]) }.toTypedArray(), plane)
        println("Signed distances to PCA plane: ${signed.joinToString(", ")}")

        // === 텍스처(무지개 + 등고선) ===
        val tex = makeHeatmapWithContours(ZgColor, vmin, vmax, nContours, contourPx, contourColor)

        // === 평면 메쉬 (Z를 Up으로) ===
        val mesh = buildPlaneMeshWithTextureZUp(Xg, Yg, ZgHeight, zScale)
        ...
        val markers = Group().apply {
            for (i in points.indices) {
                ...
                val zOnPlane = -(nx * px + ny * py + plane.d) / nz
                val zUp = zOnPlane * zScale + 0.5
                children += makeSphereZUp(px, py, zUp, 2.5, Color.BLACK)
            }
        }

        // === 축/그리드 ===
        val axisLength = max(xi.last() - xi.first(), yi.last() - yi.first())
        val axes = Axes.buildAxes(
            length = axisLength,
            tickStep = axisLength / 10.0,
            axisRadius = max(0.5, axisLength * 0.006),
            showGrid = true,
            gridStep = axisLength / 10.0
        )
        ...
        val camera = PerspectiveCamera(true).apply {
            ...
            transforms.addAll(
                ...
                Translate(0.0, 0.0, -getCameraDistanceZUp(xi, yi, ZgHeight, zScale))
            )
        }
        ...
        // === Scene (HUD 오버레이) ===
        val hud = Axes.buildAxisLegendHud(Group()) // Axes.kt의 텍스트를 "Z (Up)"로 바꿔주세요
        val rootPane = StackPane(subScene, hud)
        ...
        primaryStage.title = "PCA Average Plane + Rainbow Contours (Z-Up, devi color)"
        primaryStage.scene = scene
        primaryStage.show()
    }

    // ---------- 유틸 (Z-Up 대응) ----------
    private fun buildPlaneMeshWithTextureZUp(
        X: Array<DoubleArray>, Y: Array<DoubleArray>, Z: Array<DoubleArray>, zScale: Double
    ): TriangleMesh {
        val rows = X.size
        val cols = X[0].size
        val mesh = TriangleMesh()

        // 포인트: (x, y, zUp) — 여기서 zUp이 높이
        for (i in 0 until rows) {
            for (j in 0 until cols) {
                val xf = X[i][j].toFloat()
                val yf = Y[i][j].toFloat()
                val zUp = (Z[i][j] * zScale).toFloat()
                mesh.points.addAll(xf, yf, zUp)
            }
        }
    ...
    private fun makeSphereZUp(x: Double, y: Double, zUp: Double, radius: Double, color: Color): Node {
    ...
    private fun getCameraDistanceZUp(
        xi: DoubleArray, yi: DoubleArray, Zg: Array<DoubleArray>, zScale: Double
    ): Double {
        val dx = xi.last() - xi.first()
        val dy = yi.last() - yi.first()
        var minZ = Zg[0][0]; var maxZ = Zg[0][0]
        for (r in Zg.indices) for (c in Zg[r].indices) {
            minZ = min(minZ, Zg[r][c]); maxZ = max(maxZ, Zg[r][c])
        }
        val dz = (maxZ - minZ) * zScale
        return max(dx, max(dy, dz)) * 2.5 + 400.0
    }
